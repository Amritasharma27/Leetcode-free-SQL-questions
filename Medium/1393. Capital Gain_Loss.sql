...
with gain_loss as(
    select stock_name,
    sum(case when operation='Buy' then price end) as buying_price,
    sum(case when operation= 'sell' then price end) as selling_price
from stocks
group by stock_name)

select stock_name, (selling_price-buying_price) as capital_gain_loss
from gain_loss
